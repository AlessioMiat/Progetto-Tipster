...
//
// L'altezza di ogni card segue le proporzioni reali dello screenshot (non un
// crop fisso): confrontato con un crop "cover" a altezza fissa, cosi' non si
// taglia mai via l'intestazione o il fondo della schedina (deciso 13/07/2026
// dopo un test con screenshot veri).
...
async function generaImmagineRecap(vinte, screenshotBuffers) {
  const cardW = W - PAD * 2;
  const imgW = cardW - IMG_INSET * 2;

  const metas = await Promise.all(screenshotBuffers.map(b => sharp(b).metadata()));
  const imgHeights = metas.map(m => Math.round((imgW * m.height) / m.width));
  const tileHeights = imgHeights.map(h => h + IMG_INSET * 2 + LABEL_H);

  const tileYs = [];
  let y = PAD + HEADER_H;
  tileHeights.forEach(h => {
    tileYs.push(y);
    y += h + TILE_GAP;
  });
  const totalH = y - TILE_GAP + FOOTER_H + PAD;

  let cards = "";
  vinte.forEach((v, i) => {
    const tileY = tileYs[i];
    const labelTop = tileY + IMG_INSET + imgHeights[i];
    cards += `
      <rect x="${PAD}" y="${tileY}" width="${cardW}" height="${tileHeights[i]}" rx="18" fill="#1c150d" stroke="rgba(224,170,62,0.35)" stroke-width="1"/>
  ...
  for (let i = 0; i < vinte.length; i++) {
    const shot = await sharp(screenshotBuffers[i]).resize(imgW, imgHeights[i], { fit: "fill" }).png().toBuffer();
    composite.push({ input: shot, top: tileYs[i] + IMG_INSET, left: PAD + IMG_INSET });
    composite.push({ input: badgeBuffer, top: tileYs[i] + IMG_INSET + 12, left: PAD + cardW - IMG_INSET - 112 });
  }
